...
#         required_fields = ['TaskID', 'UserID', 'title', 'description', 'start_datetime', 'end_datetime', 'repetition', 'priority', 'status']
#         for field in required_fields:
#             if field not in data:
#                 raise ValueError(f"Missing required field: {field}")

#         # التحقق من القيم وإعدادها
#         for key, value in data.items():
...
class Task:
    ...
    def add_task(data):
        # التحقق من وجود الحقول المطلوبة
        required_fields = ['UserID', 'title', 'description', 'start_datetime', 'end_datetime', 'repetition', 'priority', 'status']
        for field in required_fields:
            if field not in data:
                raise ValueError(f"Missing required field: {field}")
    ...
    def get_tasks_by_user_id(user_id):
        try:
            tasks_cursor = Task.collection.find({"UserID": user_id})
            tasks_list = []
            for task in tasks_cursor:
                task["_id"] = str(task["_id"])
                tasks_list.append(task)
